...
namespace CodeWalker.Project.Panels
{
    public partial class GenerateNavMeshPanel : ProjectPanel
    {
        ...
        public GenerateNavMeshPanel(ProjectForm projectForm)
        {
            ...
            // ДОБАВЛЕННЫЙ КОД: Кнопка для автоматического захвата зоны
            Button btnCam = new Button();
            btnCam.Text = "Захватить зону (100м)";
            btnCam.Location = new System.Drawing.Point(280, 129);
            btnCam.Size = new System.Drawing.Size(150, 48);
            btnCam.Click += (s, e) => {
                if (ProjectForm?.WorldForm != null) {
                    var cam = ProjectForm.WorldForm.GetCameraPosition();
                    MinTextBox.Text = $"{(int)(cam.X - 100)}, {(int)(cam.Y - 100)}";
                    MaxTextBox.Text = $"{(int)(cam.X + 100)}, {(int)(cam.Y + 100)}";
                }
            };
        ...
        private void GenerateButton_Click(object sender, EventArgs e)
        {
            ...
            Vector2 userMin = FloatUtil.ParseVector2String(MinTextBox.Text);
            Vector2 userMax = FloatUtil.ParseVector2String(MaxTextBox.Text);
            ...
            if ((userMin.X < -6000) || (userMin.Y < -6000) || (userMax.X > 9000) || (userMax.Y > 9000))//it's over 9000
            {
            ...
            bool excludeProjectProps = ExcludeProjectPropsCheckBox.Checked;
            List<PropExclusionAabb> propExclusions = null;
            if (excludeProjectProps)
            {
                UpdateStatus("Collecting project entities to exclude...");
                propExclusions = BuildProjectEntityExclusionBoxes(ProjectForm.CurrentProjectFile, userMin, userMax);
            }

            float density = 1.5f; // Увеличено для возможности генерации больших кусков карты
            ...
            var layers = new[] { true, true, false }; // collision layers: bounds, entities (включаем интерьеры/маппинг), water

            int hitTestCount = 0; //statistic for number of hit tests done
            int hitCount = 0;//statistic for total ray hits
            int newCount = 0;//statistic for total nav polys added to builder

            var propExclusionsSnapshot = propExclusions != null ? propExclusions.ToArray() : Array.Empty<PropExclusionAabb>();

            float chunkSizeMeters = (float)ChunkSizeNavUpDown.Value;
            ...
            Task.Run(() =>
            {
                ...
                int ncx = (int)Math.Ceiling((userMax.X - userMin.X) / chunkSz);
                int ncy = (int)Math.Ceiling((userMax.Y - userMin.Y) / chunkSz);
                int totalChunks = Math.Max(1, ncx * ncy);
                int chunkIndex = 0;

                for (float x0 = userMin.X; x0 < userMax.X; x0 += chunkSz)
                {
                    for (float y0 = userMin.Y; y0 < userMax.Y; y0 += chunkSz)
                    {
                        var coreMin = new Vector2(x0, y0);
                        var coreMax = new Vector2(Math.Min(x0 + chunkSz, userMax.X), Math.Min(y0 + chunkSz, userMax.Y));
                        if (coreMax.X <= coreMin.X || coreMax.Y <= coreMin.Y) continue;

                        var gridMin = new Vector2(Math.Max(userMin.X, coreMin.X - overlap), Math.Max(userMin.Y, coreMin.Y - overlap));
                        var gridMax = new Vector2(Math.Min(userMax.X, coreMax.X + overlap), Math.Min(userMax.Y, coreMax.Y + overlap));
                        if (gridMax.X <= gridMin.X || gridMax.Y <= gridMin.Y) continue;

                        chunkIndex++;
                        UpdateStatus($"Chunk {chunkIndex}/{totalChunks} ({coreMin.X:0},{coreMin.Y:0}) …");
                ...
                var vertexCountXY = (gridMax - gridMin) / density;
                ...
                var bmin = new Vector3(gridMin, 0);
                var bmax = new Vector3(gridMax, 0);
                ...
                for (int vx = 0; vx < vertexCountX; vx++)
                {
                    for (int vy = 0; vy < vertexCountY; vy++)
                    {
                        ...
                        while (intres.Hit)
                        {
                            if (intres.HitDist > 0)
                            {
                                // ПРОВЕРКА ВЫСОТЫ: пускаем луч вверх от пола на 2.2 метра (рост зомби + запас)
                                // Если луч во что-то упирается, значит тут тесно (низкий потолок/объект), пропускаем точку
                                Ray upRay = new Ray(intres.Position + new Vector3(0, 0, 0.1f), new Vector3(0, 0, 1));
                                var upRes = space.RayIntersect(upRay, 2.2f, layers);
                                
                                if (!upRes.Hit) // Если над головой свободно
                                {
                                    if (excludeProjectProps && propExclusionsSnapshot.Length > 0 &&
                                        IsInsidePropExclusion(intres.Position, propExclusionsSnapshot))
                                    {
                                        // Surface is inside a project entity (prop) bounds — skip, continue ray downward.
                                        ray.Position.Z = intres.Position.Z - 0.5f;
                                    ...
                                    if (vgrid.CurVertexCount > 15)
                                    { break; }
                                }
                            }
                            // ШАГ ВНИЗ: изменен с 3.0f на 0.5f. 
                            // Теперь генератор не будет проскакивать тонкие полы/потолки в новых MLO.
                            ray.Position.Z = intres.Position.Z - 0.5f;
                            intres = space.RayIntersect(ray, float.MaxValue, layers);
                        }
                        vgrid.EndCell(vx, vy);
                ...
                {
                    if (poly.Vertices == null) continue;
                    var cen = poly.GetCenter();
                    if (cen.X < coreMin.X || cen.X >= coreMax.X || cen.Y < coreMin.Y || cen.Y >= coreMax.Y) continue;
        ...
        private struct PropExclusionAabb
        {
            public Vector3 Min;
            public Vector3 Max;
        }
        ...
        /// <summary>
        /// World-space boxes for entities in the project's ymaps. Used to skip navmesh samples inside placed props.
        /// </summary>
        private static List<PropExclusionAabb> BuildProjectEntityExclusionBoxes(ProjectFile project, Vector2 genMin, Vector2 genMax)
        {
            const float padXY = 0.45f;
            const float padZ = 0.35f;
            var list = new List<PropExclusionAabb>();
            if (project?.YmapFiles == null) return list;

            float gminX = Math.Min(genMin.X, genMax.X) - padXY;
            float gminY = Math.Min(genMin.Y, genMax.Y) - padXY;
            float gmaxX = Math.Max(genMin.X, genMax.X) + padXY;
            float gmaxY = Math.Max(genMin.Y, genMax.Y) + padXY;
            ...
            foreach (var ymap in project.YmapFiles)
            {
                ...
                foreach (var ent in ymap.AllEntities)
                {
                    ...
                    // Interior shell / room entity — bounds are huge; do not carve the whole MLO from nav.
                    if (ent.Archetype is MloArchetype) continue;

                    if (!TryGetEntityWorldAabb(ent, out Vector3 emin, out Vector3 emax)) continue;

                    emin -= new Vector3(padXY, padXY, padZ);
                    emax += new Vector3(padXY, padXY, padZ);

                    if (emax.X < gminX || emin.X > gmaxX || emax.Y < gminY || emin.Y > gmaxY) continue;

                    list.Add(new PropExclusionAabb { Min = emin, Max = emax });
                }
            }
        ...
        private static bool TryGetEntityWorldAabb(YmapEntityDef ent, out Vector3 emin, out Vector3 emax)
        {
            emin = default;
            emax = default;
            const float maxHoriz = 85f;
            const float maxVert = 130f;

            if (ent.Archetype != null)
            {
                var ext = ent.BBMax - ent.BBMin;
                if (ext.LengthSquared() > 1e-4f)
                {
                    if (ext.X > maxHoriz || ext.Y > maxHoriz || ext.Z > maxVert) return false;
                    emin = ent.BBMin;
                    emax = ent.BBMax;
                    return true;
                }
            ...
            if (r <= 0.05f && ent.Archetype != null) r = ent.Archetype.BSRadius;
            if (r <= 0.05f) r = 0.75f;
            if (r > 45f) return false;

            var p = ent.Position;
            float zLo = Math.Max(r * 0.35f, 0.5f);
            float zHi = Math.Max(r * 1.25f, 2.5f);
            emin = new Vector3(p.X - r, p.Y - r, p.Z - zLo);
            emax = new Vector3(p.X + r, p.Y + r, p.Z + zHi);
            return true;
        }

        private static bool IsInsidePropExclusion(Vector3 p, PropExclusionAabb[] boxes)
        {
